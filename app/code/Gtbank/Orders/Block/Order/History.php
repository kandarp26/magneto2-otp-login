<?php
namespace Gtbank\Marketplace\Block\Order;
...
class History extends \Magento\Framework\View\Element\Template
{
    ...
    public function getAllSalesOrder()
    {
        ...
        if (!$this->salesOrderLists) {
            ...
            if (isset($paramData['name'])) {
                $filterName = $paramData['name'] != "" ? $paramData['name'] : "";
            }

            if (isset($paramData['product_name'])) {
                $filterProductName = $paramData['product_name'] != "" ? $paramData['product_name'] : "";
            }

            if (isset($paramData['email'])) {
                $filterEmail = $paramData['email'] != "" ? $paramData['email'] : "";
            }

            $orderids = $this->getOrderIdsArray($customerId, $filterOrderstatus);

            $ids = $this->getEntityIdsArray($orderids, $filterProductName);
            ...
            if (isset($paramData['sort'])) {
                if ($paramData['sort'] == 'asc' || $paramData['sort'] == 'desc') {
                    $collection->setOrder(
                    'created_at',
                    $paramData['sort']
                );
            ...
            if ($filterName) {
                $collection->getSelect()->where('so.customer_firstname like "%' . $filterName . '%"');
            }

            if ($filterEmail) {
                $collection->getSelect()->where('so.customer_email like "%' . $filterEmail . '%"');
            }
    ...
    public function getEntityIdsArray($orderids = [], $filterProductName='')
    {
        ...
        foreach ($orderids as $orderid) {
            ...
            if ($filterProductName) {
                $collectionIds->addFieldToFilter(
                    'magepro_name',
                    ['like' => '%' . $filterProductName . '%']
                );
    ...
    public function getMainOrder($orderId, $productName='')
    {
        $sellerId = $this->getCustomerId();
        $collection = $this->_objectManager->create(
